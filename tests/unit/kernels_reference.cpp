...
void deepworks::reference::CPULinearForward(const float* X, const float* W, float* result,
    ...
    deepworks::reference::Multi(X, W, result, batch_size, in_features, out_features);
}
...
void deepworks::reference::CPULinearBackward(const float* input, const float* W, const float* dx, float* dW, float* grad_output,
    ...
    deepworks::reference::Multi(inputT.data(), dx, dW, in_features, batch_size, out_features);
    ...
    deepworks::reference::Multi(dx, WT.data(), grad_output, batch_size, out_features, in_features);
}
...
void deepworks::reference::Multi(const float* in1, const float* in2, float* out, size_t m, size_t n, size_t l) {
