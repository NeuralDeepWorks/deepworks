...
static dw::Model buildIrisModel(int batch_size) {
    int in_features  = 4;
...
int main(int argc, char *argv[]) {
    ...
    auto train_dir   = root + "train/iris.csv";
    auto test_dir    = root + "test/iris.csv";
    ...
    // Define model
    auto model = mode == "test" ? dw::load(model_path) : buildIrisModel(batch_size);
    model.compile();
    ...
    dw::DataLoader val_loader(std::make_shared<custom::IrisCSVDataset>(test_dir) , batch_size, /*shuffle */ false);
    ...
    dw::DataLoader train_loader(std::make_shared<custom::IrisCSVDataset>(train_dir), batch_size, /*shuffle */ true);
